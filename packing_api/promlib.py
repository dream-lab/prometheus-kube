...
def machine_total(hosts):
    ...
    def network(host):
        host_node = '"' + host + '"'
        query = 'node_network_speed_bytes{{ instance={} }} * 8/(10^6)'.format(host_node)
        # print(query)
        response = requests.get(prometheus + '/api/v1/query', params={
            'query': query})
        # print(response.json())
...
def machine_current(hosts):
    ...
    def network(host):
        ...
        query = '(irate(node_network_receive_bytes_total{{ instance={},device!="lo" }}[1m])' \
                '+ irate(node_network_transmit_bytes_total{{ instance={},device!="lo" }}[1m])) *8/ (10^6)'.format(
            host_node, host_node)
        # print(query)
        response = requests.get(prometheus + '/api/v1/query', params={
            'query': query})
...
def machine_average(hosts):
    ...
    def network(host):
        ...
        query = '(avg by (device) (rate(node_network_receive_bytes_total{{ instance={},device!="lo" }} [{}]))' \
                '+ avg by (device) (rate(node_network_transmit_bytes_total{{ instance={},device!="lo" }} [{}])))* 8 /(10^6)'.format(
...
def application_average(app_list):
    ...
    def network(pod):
        query='((quantile_over_time( {}, sum by (container_label_io_kubernetes_pod_name)' \
              ...
              '(rate(container_network_transmit_bytes_total{{ container_label_io_kubernetes_pod_name={},interface=~"eth0|ens.*" }}[{}]))[{}:]))) *8/(10^6)'\
...
def application_average(app_list):
    ...
    def network(pod):
        query = '(sum(avg(rate(container_network_transmit_bytes_total{{ container_label_io_kubernetes_pod_name={},interface=~"eth0|ens.*" }}[{}])))' \
                '+ sum(avg(rate(container_network_receive_bytes_total{{ container_label_io_kubernetes_pod_name={},interface=~"eth0|ens.*" }}[{}])))) * 8 / (10^6)' \
